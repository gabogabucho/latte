...
| `npm test` | 1 | `6 failed / 31 passed files; 9 failed / 290 passed tests`. Fallos: agents (2, AgentHub peak/login), chat (1, shim Windows), codex (1, binario Windows), detect (1, .exe Windows), mcp (3, CLI runtime), service (1, Engram CLI ausente en PATH). Resto verde. |
...
| `npm run dev` (ventana, kill = Ctrl+C) | 143 (SIGTERM, cierre manual) | Ventana Electron ABRIO OK (main + renderer + gpu vivos, verificado con `ps`); cerrada con kill. Sin fallo sandbox → no se añaden flags. Solo avisos GPU no fatales (MESA-INTEL Haswell Vulkan incomplete; `libva iHD_drv_video.so init failed`). |
