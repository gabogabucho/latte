...
export function DocumentExplorer({documents,workId,selectedId,onSelect,onCreate,busy}: {documents:WorkDocument[];workId:string;selectedId:string|null;onSelect:(id:string)=>void;onCreate:()=>void;busy:boolean}) {
 ...
 useEffect(()=>{
  ...
  setChecking(true);
  // Collapsed means nobody is looking: stop reading the disk until it opens again.
  const tick=async()=>{if(stopped||inFlight||!open||document.hidden)return;inFlight=true;setChecking(true);
 ...
 return <section className={'document-explorer'+(open?'':' collapsed')} aria-label="Explorador de documentos">
  <header className="explorer-heading"><div><span className="eyebrow">ESPACIO DE CAMPAÑA</span><h2><button className="explorer-collapse" aria-expanded={open} aria-label={open?'Plegar documentos':'Desplegar documentos'} onClick={()=>setOpen(o=>!o)}><ChevronDown size={17} className={open?'rotated':''}/></button>Documentos <small>{documents.length}</small></h2></div><button onClick={onCreate} disabled={busy}><Plus size={14}/>Documento</button></header>
  {open&&<>
