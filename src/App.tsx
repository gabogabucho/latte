...
export function App() {
  ...
  const restartMember = (memberId: string) => run(async () => { await api.restartTeamMember(memberId); dropChat(memberId); if (work) await loadTeam(work.id); setNotice('Conversación nueva. El agente vuelve a leer el contexto del trabajo al abrirla.'); });
