...
export function App() {
  ...
  return <div className={'app-shell' + (dragging ? ' dragging' : '')} style={{ ['--agent-width' as string]: `${agentWidth}px` }}>
    ...
    {(modal === 'brand' || modal === 'work') && <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget && !busy) setModal(null); }}><section role="dialog" aria-modal="true" aria-labelledby="dialog-title" className="modal"><button className="modal-close" aria-label="Cerrar" onClick={() => setModal(null)}><X size={20} /></button><div className="document-kicker">TU ESTUDIO, CON ORDEN</div><h2 id="dialog-title">{modal === 'brand' ? 'Una nueva marca.' : 'Un nuevo trabajo.'}</h2><form onSubmit={e => { e.preventDefault(); void create(); }}><label className="field-label" htmlFor="new-name">{modal === 'brand' ? 'NOMBRE DE LA MARCA' : 'TÍTULO DEL TRABAJO'}</label><input autoFocus id="new-name" maxLength={120} value={name} onChange={e => setName(e.target.value)} placeholder={modal === 'brand' ? 'Ej. Casa Oliva' : 'Ej. Investigación de audiencia'} /><p className="footnote">Podés empezar con lo que sabés e incorporar contexto después.</p><button className="primary" disabled={!name.trim() || busy}>Crear {modal === 'brand' ? 'marca' : 'trabajo'}<ArrowUpRight size={16} /></button></form></section></div>}
