/**
 * Bounded live smoke of the Codex chat path through the real backend:
 * temp data dir, one brand/work, primary agent = Codex (system profile),
 * one tiny prompt, wait for the reply. Uses the user's existing Codex (ChatGPT)
 * login; Latte never touches credentials.
 *
 *   npx tsx scripts/smoke-codex.ts
 */
...
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'latte-smoke-codex-'));
...
async function main(): Promise<void> {
  ...
  try {
    ...
    result.codex = { installed: codex?.installed, version: codex?.version, systemLoggedIn: codex?.accounts[0]?.loggedIn, systemDetail: codex?.accounts[0]?.detail };
    if (!codex?.installed || !codex.accounts[0]?.loggedIn) {
      result.outcome = 'skipped: Codex not installed or not logged in';
      return;
    }
    const brand = await backend.service.createBrand('Smoke');
    const work = await backend.service.createWork(brand.id, 'Prueba Codex');
    result.primary = await backend.service.setPrimaryAgent({ runtime: 'codex', model: null, accountId: 'system' });
    ...
    result.persistedSessionId = Boolean(backend.repo.getChatSession(work.id, 'codex'));
...
main()
  ...
  .finally(async () => {
    ...
    fs.writeFileSync(path.resolve('scripts', 'smoke-codex-result.json'), JSON.stringify(result, null, 2));
